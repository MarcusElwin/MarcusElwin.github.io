...
Also looking at the [LLM Leaderboard](https://huggingface.co/blog/gemma) Gemma is ranking highly in comparision to other 7B models. While be interesting to see when we start to see some more fine-tuned versions of Gemma.
...
* It will be interesting to see what fine-tuned models that will start to pop up in a while.
...
### Why it interests me
The demo videos using a fairly simple prompt look very impressive, and I think there are a bunch of use cases for gaming, marketing and filmmaking with something like SORA. After it has been "battle" tested of course. The ability to simulate and understand the real world (at least as an initial foundation) is very interesting if and when we do end up with AGI.
...
### What
On February, 20th, 2024 Predibase announced that they had fine-tuned a bunch of Open-source LLMs (25 to be exact), based on Mistral. Where these fine-tuned models managed to beat GPT-4 on a couple of benchmarks and domains. The remarkable thing here is that serving of these models all work on a single GPU. 
...
2. Make it easier for ML developers to try different models will parsing results in a similar way.
